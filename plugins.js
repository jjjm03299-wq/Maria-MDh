...
// ------------------------------------------------------
// PIN LOCK CONFIGURATION & MANAGEMENT
// ------------------------------------------------------
const pinFilePath = './Gallery/database/pin.json'

function getPinData() {
    if (!fs.existsSync(pinFilePath)) {
        const defaultData = { pin: null, attempts: 0, lockoutUntil: 0 }
        fs.writeFileSync(pinFilePath, JSON.stringify(defaultData, null, 2))
        return defaultData
    }
    return JSON.parse(fs.readFileSync(pinFilePath))
}

function savePinData(data) {
    fs.writeFileSync(pinFilePath, JSON.stringify(data, null, 2))
}

async function verifyPinLock() {
    ...
    if (pinData.lockoutUntil && now < pinData.lockoutUntil) {
        ...
        process.exit(0)
    } else if (pinData.lockoutUntil && now >= pinData.lockoutUntil) {
        pinData.attempts = 0
        pinData.lockoutUntil = 0
        savePinData(pinData)
    }
    ...
    if (!pinData.pin) {
        ...
        if (!newPin || newPin.trim() === '') {
            console.log(chalk.red('PIN cannot be empty.'))
            process.exit(0)
        }
        let confirmPin = await question(chalk.green('Confirm new PIN: '))
        if (newPin !== confirmPin) {
            console.log(chalk.red('PINs do not match! Exiting.'))
            process.exit(0)
        }
        pinData.pin = newPin
        pinData.attempts = 0
    ...
    while (true) {
        ...
        if (enteredPin === '1') {
            console.log(chalk.red('Exiting bot session.'))
            process.exit(0)
        }

        if (enteredPin === pinData.pin) {
            console.log(chalk.green('Access Granted! Starting Maria-md...\n'))
            break
        } else {
            pinData.attempts += 1
            if (pinData.attempts >= 3) {
                ...
                console.log(chalk.red(`Incorrect PIN! Attempts left: ${3 - pinData.attempts}`))
                savePinData(pinData)
            }
        }
    }
